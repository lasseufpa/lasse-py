...
Python to Matlab: write HDF5 files to be read from Matlab with our companion function
readArrayFromHDF5.m.
...
def read_several_matlab_arrays_from_mat(fileName, arrayNames):
    """
    Read an array saved in Matlab with something like:
    save -v6 'test.mat' arrayName1 arrayName2 arrayName3
    or
    ...
    Note that this gives support to multidimensional complex-valued arrays.
    :param fileName: file name (test.mat in this example)
    :param arrayNames: list or tuple with name of arrays of interest
    :return: list with numpy arrays
    """
    arrays = spio.loadmat(fileName)
    x = list()
    for i in range(len(arrayNames)):
        x.append(arrays[arrayNames[i]])
        # return np.transpose(x) #scipy already transposes. No need to:
        # https://stackoverflow.com/questions/21624653/python-created-hdf5-dataset-transposed-in-matlab
    return x
...
def test_read_matlab_array_from_mat():
    ...
    if False:
        ...
        if True:
            ...
            print(
                x[50, 6, 3, 1, 3, 40]
            )  # and compare with allHs(50+1,6+1,3+1,1+1,3+1,40+1) in Matlab
        else:
...
def test_read_matlab_array_from_mat2():
    fileName = "D:/gits/lasse/software/2019-aldebaro-ita-fwrl/outputs/channels_f3_b12tauc190taup30e_10.mat"
    arrayName = "H"
    H = read_matlab_array_from_mat(fileName, arrayName)
    print(H.shape)
    print(H[1, 2, 3, 1])
...
if __name__ == "__main__":
    ...
    test_read_matlab_array_from_mat2()
